"""Tests for build_event in mosaic_logger_core (ghcp-cli variant).

Covers: required envelope fields (harness must be "ghcp-cli"), optional context
fields, the degrade-never-fabricate rule (None/empty-string/empty-collection
produce absent keys; False and 0 are kept), and recursive pruning of nested
dicts whose resolved sub-fields all vanish.
"""
...
class TestEnvelopeRequiredFields(unittest.TestCase):
    ...
    def test_harness_is_ghcp_cli(self):
        """The harness field must be 'ghcp-cli', not 'claude-code'."""
        result = core.build_event("session_start", self.ctx)
        self.assertEqual("ghcp-cli", result["harness"])

    def test_harness_is_not_claude_code(self):
        """Sanity check: the ghcp-cli adapter must not report itself as claude-code."""
        result = core.build_event("session_start", self.ctx)
