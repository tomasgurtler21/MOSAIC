"""Tests for run_id extraction in mosaic_logger_runstate (ghcp-cli variant).

The MOSAIC run_id format ({YYYYMMDD}T{HHMMSS}Z-{4hex}) is harness-independent.
The same regex patterns are used across both claude-code and ghcp-cli variants.

Covers: extract_run_id structured-match from JSON key-value, plain key-value
(colon and equals forms), bare-match fallback, structured-match priority over
bare match, near-miss cases (wrong format, uppercase hex, partial pattern),
no-match cases (None, empty string, plain text), and the never-raises contract.
"""
...
class TestExtractRunIdStructuredMatch(unittest.TestCase):
    ...
    def test_extracts_from_ghcp_cli_subagent_start_prompt(self):
        """Extraction works from the agentPrompt field content of a subagentStart event."""
        prompt = (
            '{"agent_instance_id": "contracts-designer#12", '
            f'"run_id": "{VALID_RUN_ID}", '
            '"task_description": "Design interfaces for ghcp-cli adapter."}'
        )
        result = runstate.extract_run_id(prompt)
        self.assertEqual(VALID_RUN_ID, result)

    def test_extracts_from_ghcp_cli_subagent_stop_response(self):
        """Extraction works from the response field content of a subagentStop event."""
        response = (
            '{"agent_instance_id": "contracts-designer#12", '
            '"status_code": "SUCCESS", '
            f'"run_id": "{VALID_RUN_ID}", '
