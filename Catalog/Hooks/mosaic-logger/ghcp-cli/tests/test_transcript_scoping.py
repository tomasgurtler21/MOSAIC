"""Tests for transcript path scoping in the degradation bucket (ghcp-cli adapter).
...
class TestHarnessConstant(unittest.TestCase):
    ...
    def test_harness_constant_is_ghcp_cli(self):
        self.assertEqual("ghcp-cli", core.HARNESS)
...
class TestTranscriptScopeSegment(unittest.TestCase):
    ...
    def test_returns_non_empty_string(self):
        result = core.transcript_scope_segment("ghcp-cli", "sess-abc-123")
        self.assertIsInstance(result, str)
        self.assertTrue(result)

    def test_separates_harness_and_session_with_double_underscore(self):
        result = core.transcript_scope_segment("ghcp-cli", "sess-abc-123")
        self.assertEqual("ghcp-cli__sess-abc-123", result)

    def test_replaces_dots_in_session_id_with_underscores(self):
        result = core.transcript_scope_segment("ghcp-cli", "sess.with.dots")
        self.assertEqual("ghcp-cli__sess_with_dots", result)
    ...
    def test_scope_segment_contains_no_dots(self):
        result = core.transcript_scope_segment("ghcp-cli", "a.b.c.d.e")
        self.assertNotIn(".", result)

    def test_does_not_contain_path_separator_from_session_id(self):
        result = core.transcript_scope_segment("ghcp-cli", "sess/sub/path")
        self.assertNotIn("/", result)
        self.assertNotIn("\\", result)

    def test_sanitizes_reserved_chars_in_session_id(self):
        result = core.transcript_scope_segment("ghcp-cli", 'sess<with>bad:chars"here')
        for ch in '<>:"/\\|?*':
    ...
    def test_is_deterministic_for_identical_inputs(self):
        a = core.transcript_scope_segment("ghcp-cli", "sess-abc-123")
        b = core.transcript_scope_segment("ghcp-cli", "sess-abc-123")
        self.assertEqual(a, b)

    def test_produces_different_output_for_different_session_ids(self):
        a = core.transcript_scope_segment("ghcp-cli", "sess-A")
        b = core.transcript_scope_segment("ghcp-cli", "sess-B")
        self.assertNotEqual(a, b)

    def test_produces_different_output_for_different_harness_names(self):
        a = core.transcript_scope_segment("ghcp-cli", "sess-1")
...
class TestTwoSessionsProduceDistinctFiles(unittest.TestCase):
    ...
    def test_two_harnesses_for_same_session_produce_different_scope_segments(self):
        segA = core.transcript_scope_segment("ghcp-cli", "shared-session")
...
class TestHandlerCallSiteScoping(unittest.TestCase):
    """Integration: the transcript-writing call site in handle_agent_stop wires session_id through.

    These tests drive handle_agent_stop (a real lifecycle handler) with run_id
    unset so effective_run_id returns 'unknown-run', and verify that two different
    session IDs produce two distinct scoped files on disk.

    Note: ghcp-cli reads session_id from the camelCase payload field 'sessionId'.
    """
    ...
    def _make_ctx(self, session_id: str):
        """Minimal HookContext for handle_agent_stop: no run_id so bucket is used.

        ghcp-cli reads session_id from the camelCase field 'sessionId' and
        transcript_path from the camelCase field 'transcriptPath'. The
        HookContext constructor also takes event as its first argument.
        """
        payload = {
            "sessionId": session_id,
            "transcriptPath": str(self.transcript),
        }
        return core.HookContext("agentStop", payload, self.workspace, self.paths,
                                core.current_timestamp())

    def test_handle_agent_stop_writes_scoped_transcript_to_bucket(self):
        """handle_agent_stop with unknown-run bucket routes to a session-scoped path on disk."""
    ...
    def test_two_sessions_in_bucket_produce_distinct_files(self):
        """Two handle_agent_stop calls with different session IDs write to different paths."""
        import mosaic_logger_handlers_session as session_handlers
        ctxA = self._make_ctx("sess-bucket-A")
        ctxB = self._make_ctx("sess-bucket-B")
        session_handlers.handle_agent_stop(ctxA)
        session_handlers.handle_agent_stop(ctxB)
    ...
    def test_session_b_write_does_not_overwrite_session_a_file(self):
        ...
        size_before = pathA.stat().st_size
        session_handlers.handle_agent_stop(ctxB)
        # Session A's file must still exist at the same size (not overwritten).
        self.assertTrue(pathA.exists())
        self.assertEqual(size_before, pathA.stat().st_size)
