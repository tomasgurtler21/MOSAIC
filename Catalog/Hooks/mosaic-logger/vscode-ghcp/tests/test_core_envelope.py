"""Tests for build_event in mosaic_logger_core (vscode-ghcp variant).

Covers: required envelope fields (harness must be "vscode-ghcp"), optional
context fields, the degrade-never-fabricate rule (None/empty-string/empty-
collection produce absent keys; False and 0 are kept), and recursive pruning
of nested dicts whose resolved sub-fields all vanish.
"""
...
class TestEnvelopeRequiredFields(unittest.TestCase):
    ...
    def test_harness_is_vscode_ghcp(self):
        """The harness field must be 'vscode-ghcp', not 'claude-code' or 'ghcp-cli'."""
        result = core.build_event("session_start", self.ctx)
        self.assertEqual("vscode-ghcp", result["harness"])

    def test_harness_is_not_claude_code(self):
        """The vscode-ghcp adapter must not report itself as claude-code."""
        result = core.build_event("session_start", self.ctx)
        self.assertNotEqual("claude-code", result["harness"])

    def test_harness_is_not_ghcp_cli(self):
        """The vscode-ghcp adapter must not report itself as ghcp-cli."""
        result = core.build_event("session_start", self.ctx)
        self.assertNotEqual("ghcp-cli", result["harness"])
...
class TestFieldOmissionRule(unittest.TestCase):
    ...
    def test_truthy_string_kwarg_is_kept(self):
        result = core.build_event("session_start", self.ctx, model="gpt-4o")
...
class TestHarnessConstant(unittest.TestCase):
    """HARNESS module constant must be 'vscode-ghcp'."""

    def test_harness_constant_value(self):
        self.assertEqual("vscode-ghcp", core.HARNESS)

    def test_harness_constant_is_the_schema_enum_value(self):
        """build_event must use HARNESS from the module constant, not a hardcoded string."""
        result = core.build_event("run_start", _ctx())
        self.assertEqual(core.HARNESS, result["harness"])
...
class TestBuildEventJsonSerializable(unittest.TestCase):
    ...
    def test_output_is_json_serializable(self):
        ctx = _ctx()
        result = core.build_event("session_start", ctx, model="gpt-4o")
...
class TestEffectiveRunId(unittest.TestCase):
    """effective_run_id returns ctx.run_id when set, 'unknown-run' as fallback."""

    def test_returns_run_id_when_present(self):
        ctx = _ctx(run_id="20260101T170000Z-ab12")
        self.assertEqual("20260101T170000Z-ab12", core.effective_run_id(ctx))

    def test_returns_unknown_run_when_run_id_is_none(self):
        ctx = _ctx(run_id=None)
        self.assertEqual("unknown-run", core.effective_run_id(ctx))

    def test_returns_unknown_run_when_run_id_is_empty_string(self):
        ctx = types.SimpleNamespace(run_id="", timestamp="2026-01-01T00:00:00.000Z",
                                    session_id=None)
        self.assertEqual("unknown-run", core.effective_run_id(ctx))

    def test_unknown_run_is_the_literal_string(self):
        ctx = _ctx(run_id=None)
    ...
    def test_never_returns_empty_string(self):
        """effective_run_id must never return an empty string."""
        for run_id in (None, "", "unknown-run", "20260101T120000Z-ab12"):
