"""Tests for transcript path scoping in the degradation bucket (vscode-ghcp adapter).
...
class TestHarnessConstant(unittest.TestCase):
    ...
    def test_harness_constant_is_vscode_ghcp(self):
...
class TestTranscriptScopeSegment(unittest.TestCase):
    ...
    def test_returns_non_empty_string(self):
        result = core.transcript_scope_segment("vscode-ghcp", "sess-abc-123")
        self.assertIsInstance(result, str)
        self.assertTrue(result)

    def test_separates_harness_and_session_with_double_underscore(self):
        result = core.transcript_scope_segment("vscode-ghcp", "sess-abc-123")
        self.assertEqual("vscode-ghcp__sess-abc-123", result)

    def test_replaces_dots_in_session_id_with_underscores(self):
        result = core.transcript_scope_segment("vscode-ghcp", "sess.with.dots")
        self.assertEqual("vscode-ghcp__sess_with_dots", result)
    ...
    def test_scope_segment_contains_no_dots(self):
        result = core.transcript_scope_segment("vscode-ghcp", "a.b.c.d.e")
        self.assertNotIn(".", result)

    def test_does_not_contain_path_separator_from_session_id(self):
        result = core.transcript_scope_segment("vscode-ghcp", "sess/sub/path")
        self.assertNotIn("/", result)
        self.assertNotIn("\\", result)

    def test_sanitizes_reserved_chars_in_session_id(self):
        result = core.transcript_scope_segment("vscode-ghcp", 'sess<with>bad:chars"here')
        for ch in '<>:"/\\|?*':
    ...
    def test_is_deterministic_for_identical_inputs(self):
        a = core.transcript_scope_segment("vscode-ghcp", "sess-abc-123")
        b = core.transcript_scope_segment("vscode-ghcp", "sess-abc-123")
        self.assertEqual(a, b)

    def test_produces_different_output_for_different_session_ids(self):
        a = core.transcript_scope_segment("vscode-ghcp", "sess-A")
        b = core.transcript_scope_segment("vscode-ghcp", "sess-B")
        self.assertNotEqual(a, b)

    def test_produces_different_output_for_different_harness_names(self):
        a = core.transcript_scope_segment("vscode-ghcp", "sess-1")
...
class TestTwoSessionsProduceDistinctFiles(unittest.TestCase):
    ...
    def test_two_harnesses_for_same_session_produce_different_scope_segments(self):
        segA = core.transcript_scope_segment("vscode-ghcp", "shared-session")
