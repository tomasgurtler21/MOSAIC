...
same regex patterns are used across all adapter variants.
...
bare-match, near-miss and no-match cases, extract_run_id structured and bare
forms with near-miss and no-match cases, and never-raises contracts.
"""
...
class TestFallbackInstanceId(unittest.TestCase):
    ...
    def test_with_prefix_returns_prefix_underscore_timestamp_suffix(self):
        result = runstate.fallback_instance_id("TestWriter", now=BASE_TIME)
        self.assertRegex(
            result,
            r"^TestWriter_[0-9]{8}T[0-9]{6}Z-[0-9a-f]{4}$",
        )
...
class TestFallbackCallId(unittest.TestCase):
    ...
    def test_vscode_dispatch_tool_names_work_as_prefix(self):
        """VS Code uses Agent/Task/runSubagent as subagent dispatch tool names."""
        for tool_name in ("Agent", "Task", "runSubagent"):
...
class TestExtractInstanceId(unittest.TestCase):
    ...
    key-value occurrences; falls back to earliest bare occurrence; returns
    None when neither form exists."""

    def test_extracts_from_json_key_with_quoted_value(self):
        prompt = '{"agent_instance_id": "TestWriter#5", "task": "write tests"}'
        result = runstate.extract_instance_id(prompt)
        self.assertEqual("TestWriter#5", result)

    def test_extracts_from_plain_key_colon_value(self):
        prompt = "agent_instance_id: ContractsDesigner#12"
        result = runstate.extract_instance_id(prompt)
        self.assertEqual("ContractsDesigner#12", result)
    ...
    def test_structured_match_preferred_over_earlier_bare_match(self):
        prompt = 'BareAgent#5 mentioned first. {"agent_instance_id": "StructuredAgent#2"}'
        result = runstate.extract_instance_id(prompt)
    ...
    def test_extracts_from_vscode_ghcp_dispatch_prompt(self):
...
class TestExtractRunIdStructuredMatch(unittest.TestCase):
    ...
    def test_extracts_from_vscode_ghcp_subagent_start_prompt(self):
        """Extraction works from prompt in a SubagentStart dispatch."""
        prompt = (
            '{"agent_instance_id": "contracts-designer#12", '
            f'"run_id": "{VALID_RUN_ID}", '
            '"task_description": "Design interfaces."}'
        )
        result = runstate.extract_run_id(prompt)
        self.assertEqual(VALID_RUN_ID, result)

    def test_extracts_from_vscode_ghcp_subagent_stop_response(self):
        """Extraction works from last_assistant_message in a SubagentStop event."""
        response = (
            '{"agent_instance_id": "contracts-designer#12", '
            '"status_code": "SUCCESS", '
            f'"run_id": "{VALID_RUN_ID}", '
        ...
        result = runstate.extract_run_id(response)
        self.assertEqual(VALID_RUN_ID, result)


class TestExtractRunIdBareMatch(unittest.TestCase):
    """extract_run_id falls back to the earliest bare format occurrence."""
