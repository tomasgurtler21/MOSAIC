...
| [**harness**](./harness/Index.md) | Implements the `HarnessAdapter` port. Includes a `FakeAdapter` for tests (scripted responses), `ClaudeCodeAdapter` (spawns the Claude Code CLI using `--permission-mode dontAsk` + `--allowedTools` derived from agent frontmatter), `GHCPCLIAdapter` (supports Blanket `--yolo` mode or Partial Allowlist `--allow-tool` mode selected per run), and `OpenCodeAdapter` (spawns OpenCode with `--auto` which converts `ask` permissions to `allow` without a per-tool allowlist). | Invoked only by **session**, always through the `domain.HarnessAdapter` port (import-boundary enforced). |
