...
| [**compat**](./compat/Index.md) | Validates that a parsed routing table is within the supported workflow subset ("admission") and resolves its EXECUTION rows into one or two contiguous execution groups. | Consumes **workflow** output; feeds **engine** (`AdmittedWorkflow`). |
| **planstages** | Reads the stage table from a Plan.md artifact and returns a validated, ordered `StageSet` (consecutive numbering from 1, no forward dependencies, optional Approach column). | Feeds **engine**; re-invoked mid-run by **session** when stage output is re-derived. |
