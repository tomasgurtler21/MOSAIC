...
//     b. On failure → attempt recovery via ExtractProtocolJSON on the raw
//        trimmed output text. If recovery succeeds, the raw trimmed text is
//        returned without error. If recovery also fails →  ErrMalformedJSON
//        wrapping the raw output text (trimmed, capped with a visible
//        truncation indicator when it exceeds the cap).
