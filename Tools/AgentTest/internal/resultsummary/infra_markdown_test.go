...
//
// These tests call RenderInternalSummary directly with VersionSummary values
// that have InfraTests populated, and assert on the rendered Markdown.
// Infrastructure failures appear only in internal-summary.md.
...
// TestRenderInternalSummary_ContainsInfrastructureFailuresSection_WithInfraTests
// verifies that RenderInternalSummary output includes both the opening and closing
// generated:infrastructure-failures markers when the input has InfraTests populated.
func TestRenderInternalSummary_ContainsInfrastructureFailuresSection_WithInfraTests(t *testing.T) {
	// Arrange
	...
	if !strings.Contains(output, "<!-- generated:infrastructure-failures -->") {
		t.Error("RenderInternalSummary output must contain <!-- generated:infrastructure-failures --> opening marker")
	}
	if !strings.Contains(output, "<!-- /generated:infrastructure-failures -->") {
		t.Error("RenderInternalSummary output must contain <!-- /generated:infrastructure-failures --> closing marker")
	}
}

// TestRenderInternalSummary_InfraFailuresSection_ListsInfraTests verifies
// that when InfraTests is non-empty, the infrastructure-failures section
// contains the infra test's name.
func TestRenderInternalSummary_InfraFailuresSection_ListsInfraTests(t *testing.T) {
	// Arrange
	vs := minimalVersionSummaryWithInfraTests()
	...
	if blockStart < 0 || blockEnd < 0 || blockEnd <= blockStart {
		t.Fatal("generated:infrastructure-failures block not found in rendered output; " +
			"renderInfrastructureFailuresSection must be added to RenderInternalSummary")
	}
...
// TestRenderInternalSummary_InfraFailuresSection_IncludesTestDetails verifies
// that the infrastructure-failures section includes suite, numeric test ID,
// and test name for each infra test entry.
func TestRenderInternalSummary_InfraFailuresSection_IncludesTestDetails(t *testing.T) {
...
// TestRenderInternalSummary_InfraFailuresSection_EmptyInfraTests_ShowsNoFailuresMessage
...
func TestRenderInternalSummary_InfraFailuresSection_EmptyInfraTests_ShowsNoFailuresMessage(t *testing.T) {
...
// TestRenderInternalSummary_InfraFailuresSection_AppearsAfterProblemAreas
// verifies that the infrastructure-failures section is positioned after the
// problem-areas section and before the analysis:internal-analysis placeholder,
// as specified in the design.
func TestRenderInternalSummary_InfraFailuresSection_AppearsAfterProblemAreas(t *testing.T) {
	// Arrange
	vs := minimalVersionSummaryWithInfraTests()
	...
	analysisStart := strings.Index(output, "<!-- analysis:internal-analysis -->")
	...
	if infraStart < 0 {
		t.Fatal("<!-- generated:infrastructure-failures --> marker not found in output; " +
			"the section must be inserted between problem-areas and analysis:internal-analysis")
	}
	if analysisStart < 0 {
		t.Fatal("<!-- analysis:internal-analysis --> marker not found in output")
	}
	...
	// infrastructure-failures must come before analysis:internal-analysis.
	if infraStart > analysisStart {
		t.Errorf("infrastructure-failures section (at offset %d) must appear BEFORE <!-- analysis:internal-analysis --> (at offset %d)",
			infraStart, analysisStart)
	}
}

// TestRenderInternalSummary_InfraTestNotInProblemAreas verifies that an infra
...
func TestRenderInternalSummary_InfraTestNotInProblemAreas(t *testing.T) {
...
// TestRenderInternalSummary_InfraFailuresSection_MultipleEntries_AllListed
// verifies that when InfraTests contains multiple entries, all appear in the
// infrastructure-failures section.
func TestRenderInternalSummary_InfraFailuresSection_MultipleEntries_AllListed(t *testing.T) {
