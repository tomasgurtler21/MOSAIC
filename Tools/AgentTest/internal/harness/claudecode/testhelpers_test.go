...
// internal/harness/contract's own helper of the same purpose.
func newSandbox(t *testing.T, dir string) domain.Sandbox {
	...
	if err := os.MkdirAll(sb.SubjectDir, 0o755); err != nil {
		t.Fatalf("claudecode test: create subject dir: %v", err)
	}
	if err := os.MkdirAll(sb.ControlDir, 0o755); err != nil {
		t.Fatalf("claudecode test: create control dir: %v", err)
	}
	return sb
}
...
// testdata bundle fixtures, by directory name under testdata/claudecode/.
func fixtureBundleDir(t *testing.T, name string) string {
	t.Helper()
	abs, err := filepath.Abs(filepath.Join("..", "..", "..", "testdata", "claudecode", name))
	if err != nil {
		t.Fatalf("claudecode test: resolving fixture bundle dir: %v", err)
	}
	return abs
}
