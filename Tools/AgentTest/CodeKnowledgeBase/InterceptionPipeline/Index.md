...
The pipeline operates across three interception phases. The **pre-invocation** phase fires before a collaborator call is sent, deciding whether to substitute, rewrite, pass through, or halt the call. On harnesses that support post-interception, a **post-invocation** phase fires after the call is sent — on harnesses with reply recovery this is a bare passthrough (the real reply has not arrived yet), while on harnesses without reply recovery it carries the real reply and performs echo comparison. On harnesses with reply recovery, a **completion** phase fires when the collaborator's real reply is available, and is the point where echo fidelity is evaluated. The pipeline always answers with something the harness can use, even in the completion phase.
...
Only reached for harnesses whose interception layer has a post-invocation point (`HarnessCapabilities.SupportsPostInterception`). What happens at this point depends on whether the harness supports reply recovery (`HarnessCapabilities.SupportsReplyRecovery`):

- **`SupportsReplyRecovery = true` (e.g. the claude-code harness):** The post event fires immediately when the subject dispatches a collaborator call — it is a launch acknowledgement, not the collaborator's real reply. `Decide` returns a bare passthrough with no echo comparison, no state mutation, and no end record. The pending dispatch and in-flight entry are left intact so the completion event (below) can compare the real reply when it arrives.

- **`SupportsReplyRecovery = false`:** The post event carries the collaborator's actual reply. `Decide` recovers the pending stub by correlation token, compares the observed response with the expected stub response (`CompareEcho`: parsed as JSON, key order and whitespace ignored — a narrow code-fence tolerance is applied before parsing; surrounding prose is still a mismatch), emits an end log record carrying the echo result, and clears the in-flight entry.

**Neither branch ever halts or denies** — the collaborator has already run by this point, so refusing it could only damage the subject's turn, never prevent anything.

### One interception (completion)

Only reached on harnesses with `SupportsReplyRecovery = true`. This event fires when the dispatched collaborator has finished and its real reply is available. The shell recovers the pending stub by correlation token (populated directly from the harness's own dispatch-scoped identifier — see Correlation token in Key Concepts below). `Decide` performs the same steps as a non-reply-recovery post event: echo comparison, end record emission, in-flight clear. The completion event is the canonical point at which echo fidelity is decided on a reply-recovery harness. **The completion path never halts or denies.**
...
| Correlation token | An opaque, innocuous identifier that survives from the pre-invocation event through to the post-invocation and completion events, letting the pipeline recover the pending stub for echo comparison. On the claude-code harness the correlation token is populated directly from the `tool_use_id` field the harness sends on `PreToolUse`, `PostToolUse`, and `SubagentStop` events — it is a dispatch-scoped, vendor-issued identifier that is unguessable and carries no test-revealing vocabulary. An absent `tool_use_id` decodes to an empty token, which is a legitimate outcome for an un-stubbed dispatch (passthrough, no echo verdict, no error). No token is minted or planted in the prompt text. |
| Reply recovery | A harness-level capability (`HarnessCapabilities.SupportsReplyRecovery`) that signals the post-invocation event fires at launch (not at reply time). When true, echo comparison is deferred to the completion event. When false, the post event carries the real reply and echo comparison happens there. The capability is declared once by the adapter; no harness name appears in the decision core. |
...
| Halt reasons | Three distinct reasons a pre-invocation call can halt: early exit (invocation count threshold reached), unmatched (registry has no stub and policy says halt), extraction failed (collaborator identity couldn't be determined at all). Post-invocation and completion never halt. |
