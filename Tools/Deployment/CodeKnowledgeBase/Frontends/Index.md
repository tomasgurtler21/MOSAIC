...
`tui.Run` owns the terminal for the process lifetime. Three entry screens (harness → mode → workspace) are answered as *local* UI state before the service call even starts — they are not asked through `Interaction`. Once the third screen is done, `startService` launches `app.Service.DeployNew`/`Update` in a goroutine seeded with only `HarnessID` and `WorkspacePath` (mode-specific fields like workflows/models are intentionally left unset so the service asks for them through `Interaction`, driving the overlay screens). Every subsequent blocking `Interaction` call from the service goroutine sends a `questionMsg` over the `tea.Program`; the root model swaps in the matching overlay screen, and the overlay's `Answer()` is sent back over a reply channel, unblocking the service goroutine. Pressing Esc on any selection overlay answers it as `Cancelled`, which the service goroutine translates to `ErrSelectionCancelled`, immediately aborting the run — no further question is asked, no config is persisted, and no plan is executed. `ctrl+c` cancels the model's context and, if a question is in flight, immediately answers it as `Cancelled` so the service goroutine cannot hang.
...
| **Cancel semantics (unified rule)** | Pressing Esc on any of the five multi-select selection screens — workflows, utility agents, infrastructure agents, standalone agents, hook bundles — cancels the whole run. `ErrSelectionCancelled` is returned to the calling flow, which asks no further questions, persists no config, and executes nothing. This is entirely distinct from the skip statuses (`domain.SkippedOne`, `domain.SkippedAll`), which yield an empty selection and let the run continue. Selecting nothing explicitly (an `Answered` result with zero option IDs) is also a continuing, non-cancelling outcome. `ctrl+c` likewise cancels the run from any screen by cancelling the service's context. |
| **Cancelled terminal outcome (TUI)** | The TUI exposes three distinct terminal outcomes: success, cancelled, and failed. A cancelled run renders `"Deployment cancelled."` in warning style and is never surfaced through the failure path. Both `ErrSelectionCancelled` (any of the five selection questions) and `ErrModelSelectionCancelled` (model selection) route to this same cancelled rendering. `ErrPlanNotConfirmed` remains distinct: it restarts the selection flow rather than ending the run. |
| **CLI has no cancel keypress** | The non-interactive CLI frontend never emits `domain.Cancelled` for a selection question. A question the CLI cannot answer from its pre-supplied data resolves immediately to `SkippedOne`/`SkippedAll` with a TODO gap recorded — the run continues. This preserves CD-4 for the cancellation dimension: a TUI user who wants to proceed with no items selected for a category presses Skip rather than Esc; a CLI user omits the selection from their `--selections` file and accepts the TODO gap. Both paths produce the same proceeding outcome for equivalent inputs; only an explicit Esc (which has no CLI equivalent) aborts the run. |
