...
// mosaic_orchestrator_injections_version in the VS Code GHCP harness Frontmatter method.
//
...
//
// RED STATE:
//   TestVSCodeGHCP_Frontmatter_OrchestratorAgent_StampsMosaicOrchestratorInjectionsVersion:
//     FAILS until I4.3 updates the VS Code GHCP Frontmatter() to stamp the prefixed name.
//
...
//
//   TestVSCodeGHCP_Descriptor_KeyOrder_ContainsMosaicOrchestratorInjectionsVersionAfterMosaicInjectionsVersion:
//     FAILS until I4.4 updates the vscode-ghcp descriptor's key_order entries to the prefixed names.
...
func TestVSCodeGHCP_Frontmatter_OrchestratorAgent_StampsMosaicOrchestratorInjectionsVersion(t *testing.T) {
	mod := newModule(t)
	...
	if found == nil {
		t.Fatalf("Frontmatter(orchestrator) Set does not contain key \"mosaic_orchestrator_injections_version\"; "+
			...
			"Set keys: %v", vscodeSetKeys(plan.Set))
	}
...
// TestVSCodeGHCP_Descriptor_KeyOrder_ContainsMosaicOrchestratorInjectionsVersionAfterMosaicInjectionsVersion
// verifies that the vscode-ghcp descriptor's key_order list includes
...
//
// RED: FAILS until I4.4 updates the vscode-ghcp descriptor's key_order entries to the prefixed names.
func TestVSCodeGHCP_Descriptor_KeyOrder_ContainsMosaicOrchestratorInjectionsVersionAfterMosaicInjectionsVersion(t *testing.T) {
	mod := newModule(t)
	desc := mod.Descriptor()
