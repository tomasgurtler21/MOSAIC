...
// ---------------------------------------------------------------------------
// T3.3 — Regression: vscode-ghcp routes custom tools to main tools field (unchanged)
// ---------------------------------------------------------------------------

// TestVscodeGhcp_CustomToolDestinations_DescriptorDeclaresNone verifies that the
// vscode-ghcp descriptor does not declare a custom_tool_destination field. Only Claude
// Code declares that field; all other built-in harnesses must leave it unset so their
// custom tool routing remains unchanged.
func TestVscodeGhcp_CustomToolDestinations_DescriptorDeclaresNone(t *testing.T) {
	mod := newModule(t)
	desc := mod.Descriptor()

	if len(desc.Tools.CustomToolDestinations) != 0 {
		t.Errorf("vscode-ghcp descriptor declares CustomToolDestinations (count=%d); "+
...
// TestVscodeGhcp_CustomTool_RoutesToMainToolsNotMcpServers verifies that when an agent
// declares a custom (MCP-style) tool and the vscode-ghcp descriptor has no
// custom_tool_destination, the tool resolves to a DestMain destination and no mcpServers
// field appears in the output. This is the regression guard ensuring vscode-ghcp's custom
// tool handling is byte-identical to pre-Stage-3 behaviour.
func TestVscodeGhcp_CustomTool_RoutesToMainToolsNotMcpServers(t *testing.T) {
	mod := newModule(t)
	desc := mod.Descriptor()
	...
	// No mcpServers field must appear: vscode-ghcp has no custom_tool_destination declaration.
	for _, f := range result.Fields {
		if f.Key == "mcpServers" {
			t.Errorf("mcpServers field present in vscode-ghcp Tools() output; "+
				"vscode-ghcp must not route custom tools to a separate mcpServers field — "+
