...
// newModule constructs the Claude Code module, failing the test immediately if construction
// fails. In TDD RED phase, claudecode.New() returns an error and the test fails here.
func newModule(t *testing.T) domain.HarnessModule {
	t.Helper()
	mod, err := claudecode.New()
	if err != nil {
		t.Fatalf("claudecode.New(): %v", err) // RED: not implemented
	}
	return mod
}
