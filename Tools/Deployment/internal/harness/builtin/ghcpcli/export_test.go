...
// export_test.go exposes internal test helpers to the external ghcpcli_test package.
...
// to package ghcpcli_test but not to other packages.
...
//	<MosaicRoot>/Agents/GHCP CLI/HarnessInjections.md and
//	<MosaicRoot>/Agents/GHCP CLI/HarnessInjectionsOrchestrator.md.
func NewWithOptsForTesting(_ testing.TB, opts registry.BuiltinOptions) (domain.HarnessModule, error) {
	return New(opts)
}

// DescriptorForTesting parses the embedded ghcp-cli.yaml descriptor and
...
func DescriptorForTesting(t testing.TB) *domain.HarnessDescriptor {
	...
	if err != nil {
		t.Fatalf("DescriptorForTesting: parse embedded ghcp-cli descriptor: %v", err)
	}
	return desc
}
