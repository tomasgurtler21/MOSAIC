...
// TestCatalogFolderFlag_UtilityInfra_SpaceForm_Accepted verifies that the utility-infra
// subcommand accepts "--catalog-folder <dir>" without returning ExitUsage due to the flag.
func TestCatalogFolderFlag_UtilityInfra_SpaceForm_Accepted(t *testing.T) {
	workspace := t.TempDir()
	catalogDir := catalogFolderDir(t)
	svc := &spyService{utilityInfraResp: successSummary(workspace)}
	...
	if codeWith == cli.ExitUsage && codeWithout != cli.ExitUsage {
		t.Errorf("utility-infra returned ExitUsage with --catalog-folder but not without; "+
			"--catalog-folder must be accepted (with=%d, without=%d)",
			codeWith, codeWithout)
	}
}

// TestCatalogFolderFlag_UtilityInfra_EqualsForm_Accepted verifies the equals form on utility-infra.
func TestCatalogFolderFlag_UtilityInfra_EqualsForm_Accepted(t *testing.T) {
	workspace := t.TempDir()
	catalogDir := catalogFolderDir(t)
	svc := &spyService{utilityInfraResp: successSummary(workspace)}
	...
	if codeWith == cli.ExitUsage && codeWithout != cli.ExitUsage {
		t.Errorf("utility-infra returned ExitUsage for --catalog-folder=<dir> but not without; "+
			"both forms must be accepted (with=%d, without=%d)",
			codeWith, codeWithout)
	}
}
