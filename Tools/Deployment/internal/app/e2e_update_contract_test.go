...
//
//   (a) On create, managed region regions are filled by the appropriate generator and
//       project region regions are left empty.
//
//   (b) On update, managed region regions are regenerated (replacing any previous content),
//       and project region regions from the previously-deployed file survive byte-identically
...
// updateContractSource is a minimal agent source file that carries one managed region
// (HarnessConstraints, tool-managed) and one project region (IdentityExtension, user-owned).
// The arrangement matches the real generic agent structure so the test exercises the same
// update-mode code path that production deploys use.
...
func applyUpdateContract(t *testing.T, deployed []byte) []byte {
	...
	req := transform.Request{
		...
		Protocol: domain.ProtocolContent{Version: "1.9"}, // protocol present but no <CommunicationProtocol type="managed"> in source
		Deployed: deployed,
	}
...
// ---------------------------------------------------------------------------
// (a) Create mode — managed region filled, project region empty
// ---------------------------------------------------------------------------

// TestE2E_UpdateContract_CreateMode_DeployedRegionIsFilled verifies that on a first deploy
// (no previously-deployed file), the <HarnessConstraints type="managed"> region is filled with
// content from the harness module. The empty protocol content is acceptable here because the
// synthetic source has no <CommunicationProtocol type="managed"> region.
func TestE2E_UpdateContract_CreateMode_DeployedRegionIsFilled(t *testing.T) {
	...
	if !bytes.Contains(output, wantContent) {
		t.Errorf("create mode: <HarnessConstraints type=\"managed\"> not filled with harness content;\n"+
...
// the <IdentityExtension type="project"> region is left empty (no content between its tags).
func TestE2E_UpdateContract_CreateMode_InjectionRegionIsEmpty(t *testing.T) {
	...
	openTag := []byte("<IdentityExtension type=\"project\">")
	closeTag := []byte("</IdentityExtension>")

	openIdx := bytes.Index(output, openTag)
	if openIdx < 0 {
		t.Fatalf("create mode: <IdentityExtension type=\"project\"> open tag not found in output")
	}
	afterOpen := output[openIdx+len(openTag):]
	closeIdx := bytes.Index(afterOpen, closeTag)
	if closeIdx < 0 {
		t.Fatalf("create mode: </IdentityExtension> close tag not found after open tag")
	}
	between := afterOpen[:closeIdx]
	if len(bytes.TrimSpace(between)) > 0 {
		t.Errorf("create mode: IdentityExtension injection region is not empty on create;\n"+
...
// ---------------------------------------------------------------------------
// (b) Update mode — managed region regenerated, project region content preserved
// ---------------------------------------------------------------------------

// TestE2E_UpdateContract_UpdateMode_InjectionContentSurvivesUpdateByteIdentically verifies
// that content placed by the user inside <IdentityExtension type="project"> in a deployed file
...
func TestE2E_UpdateContract_UpdateMode_InjectionContentSurvivesUpdateByteIdentically(t *testing.T) {
	...
	if !bytes.Contains(secondOutput, []byte(userContent)) {
		t.Errorf("update mode: user-placed injection content did not survive update;\n"+
			"want %q in <IdentityExtension type=\"project\">\n\noutput (first 1200 bytes):\n%s",
...
// <HarnessConstraints type="managed"> is regenerated on update even when the deployed file
...
func TestE2E_UpdateContract_UpdateMode_DeployedRegionIsRegeneratedOnUpdate(t *testing.T) {
	...
	deployed := simulateUserEdit(t, firstOutput,
		"<HarnessConstraints type=\"managed\">", "</HarnessConstraints>",
		tampered)
	...
	if bytes.Contains(secondOutput, []byte(tampered)) {
		t.Errorf("update mode: tampered <HarnessConstraints type=\"managed\"> content survived update;\n"+
	...
	if !bytes.Contains(secondOutput, wantContent) {
		t.Errorf("update mode: <HarnessConstraints type=\"managed\"> not refilled with current harness content after update;\n"+
			"want %q in output\n\noutput (first 800 bytes):\n%s",
