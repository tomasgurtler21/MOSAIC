...
// absent.
func (f *Frontmatter) Get(key string) (domain.FieldValue, bool) {
	for _, e := range f.entries {
		if e.key == key {
			return e.value, true
		}
...
func (f *Frontmatter) Fields() []domain.FrontmatterField {
	fields := make([]domain.FrontmatterField, len(f.entries))
	for i, e := range f.entries {
		fields[i] = domain.FrontmatterField{Key: e.key, Value: e.value}
	}
	return fields
}
...
func (f *Frontmatter) Set(key string, v domain.FieldValue) error {
	for _, e := range f.entries {
		if e.key == key {
			e.value = v
			e.dirty = true
...
func (f *Frontmatter) Insert(key string, v domain.FieldValue, at Position) error {
...
func parseGroupValue(key string, lines [][]byte, startLine int) (domain.FieldValue, error) {
	firstLine := lines[0]
...
func parseBlockValue(lines [][]byte, startLine int) (domain.FieldValue, error) {
	// Skip empty lines at the start.
	for len(lines) > 0 && len(bytes.TrimSpace(lines[0])) == 0 {
		lines = lines[1:]
	}
	if len(lines) == 0 {
		return domain.ScalarValue("", domain.QuotePlain), nil
	}

	first := lines[0]
	// Tab check.
	if len(first) > 0 && first[0] == '\t' {
		return domain.FieldValue{}, fmt.Errorf(
...
func parseBlockSequence(lines [][]byte, startLine int) (domain.FieldValue, error) {
	var items []domain.FieldValue
	for i, line := range lines {
		...
		if err != nil {
			return domain.FieldValue{}, fmt.Errorf("frontmatter: line %d: %w", lineNum, err)
		}
		items = append(items, item)
	}
	return domain.ListValue(items, domain.ListBlock), nil
}

// parseBlockMapping parses indented "  key: value" lines into a KindMapping.
func parseBlockMapping(lines [][]byte, startLine int) (domain.FieldValue, error) {
	...
	for i, line := range lines {
		...
		pairs = append(pairs, domain.FieldPair{Key: key, Value: val})
	}
	return domain.MappingValue(pairs), nil
}

// parseFlowSequence parses a "[item1, item2, ...]" value into a KindList with ListFlow.
func parseFlowSequence(data []byte, startLine int) (domain.FieldValue, error) {
	// Find the matching ']'.
	end, comment, err := findFlowSequenceEnd(data, startLine)
	if err != nil {
		return domain.FieldValue{}, err
	}
	...
	result := domain.ListValue(items, domain.ListFlow)
	if len(comment) > 0 {
...
func splitFlowItems(inner []byte, startLine int) ([]domain.FieldValue, error) {
	var items []domain.FieldValue
	var current []byte
	i := 0
...
func parseScalarBytes(data []byte) (domain.FieldValue, error) {
	if len(data) == 0 {
		return domain.ScalarValue("", domain.QuotePlain), nil
	}
	...
	case '"':
		...
		if end < 0 {
			return domain.FieldValue{}, fmt.Errorf("unclosed double-quoted string")
		}
		...
		fv := domain.ScalarValue(unescaped, domain.QuoteDouble)
		fv.Comment = comment
		return fv, nil
	...
	case '\'':
		...
		if end < 0 {
			return domain.FieldValue{}, fmt.Errorf("unclosed single-quoted string")
		}
		raw := data[1 : end-1] // strip outer quotes
		...
		fv := domain.ScalarValue(unescaped, domain.QuoteSingle)
		fv.Comment = comment
		return fv, nil

	default:
		value, comment := splitPlainAndComment(data)
		fv := domain.ScalarValue(string(value), domain.QuotePlain)
		if len(comment) > 0 {
...
func serializeEntry(key string, v domain.FieldValue, nl string) string {
	switch v.Kind {
...
func serializeScalarEntry(key string, v domain.FieldValue, nl string) string {
	var sb strings.Builder
	sb.WriteString(key)
	sb.WriteString(": ")
...
func serializeListEntry(key string, v domain.FieldValue, nl string) string {
	var sb strings.Builder
...
func serializeMappingEntry(key string, v domain.FieldValue, nl string) string {
	var sb strings.Builder
	sb.WriteString(key)
	sb.WriteString(":")
	sb.WriteString(nl)
	for _, pair := range v.Pairs {
		...
		case domain.KindList:
			if pair.Value.List == domain.ListFlow {
				sb.WriteString("[")
...
func serializeScalarInline(v domain.FieldValue) string {
	switch v.Quote {
	case domain.QuoteDouble:
		return `"` + escapeDoubleQuoted(v.Scalar) + `"`
	case domain.QuoteSingle:
...
func cloneFieldValue(v domain.FieldValue) domain.FieldValue {
	result := v
	if v.Items != nil {
		result.Items = make([]domain.FieldValue, len(v.Items))
		for i, item := range v.Items {
			result.Items[i] = cloneFieldValue(item)
		}
	}
	if v.Pairs != nil {
		result.Pairs = make([]domain.FieldPair, len(v.Pairs))
		for i, p := range v.Pairs {
			result.Pairs[i] = domain.FieldPair{Key: p.Key, Value: cloneFieldValue(p.Value)}
		}
	}
	return result
}
