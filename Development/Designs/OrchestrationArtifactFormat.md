...
| `current_state` | Overwritten in place, every **workflow** step | Orchestrator, after each workflow invocation completes and on phase/stage transitions |
...
**Infrastructure invocations are excluded from `current_state`.** An infrastructure agent (checkpoint, commit, review, or restore class — see `InfrastructureAgentConcept.md`) is not a workflow participant, so its completion leaves `phase`, `stage`, `last_status`, `last_agent`, and `error_code` exactly as they were: `current_state` continues to name the last **workflow** step, never an infrastructure invocation that ran after it. This holds on disk, not only for the duration of one continuous run — an infrastructure invocation must not move, clear, or invalidate the recorded workflow position, at any point, including across interruption and resumption. The infrastructure invocation is still fully recorded: its Execution Log row is still appended, `global_sequence` still advances, `last_updated` still moves, and its output artifacts (e.g. a checkpoint log) still upsert into the Artifacts registry, exactly as for a workflow step. Only `current_state` is left untouched.

All writes to this file happen **after** an invocation completes, never before — there is no "in-progress" state to track separately. If an invocation is interrupted mid-flight, the file simply still reflects the last completed step, which is exactly what recovery (§9) relies on.
