...
- Jest testing patterns (unit tests with mocks, integration tests with supertest)
...
- Use Zod schemas for runtime validation of API inputs (co-located with or near the route/controller)
- Enums use PascalCase with UPPER_SNAKE_CASE members (e.g., `TaskStatus.IN_PROGRESS`)

#### Naming Conventions
- Files: kebab-case (e.g., `task.service.ts`, `auth.middleware.ts`)
...
- **controllers/** — Request/response handling; validate input via Zod, call service, format response
- **services/** — Business logic; use `Result<T>` pattern; no throwing
- **repositories/** — Prisma queries; encapsulate all database access
