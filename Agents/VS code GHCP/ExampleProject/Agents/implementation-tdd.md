...
### Domain Expertise
You specialize in Node.js 20 + TypeScript 5 + Express 4 backend development with deep knowledge of:
- Prisma ORM with PostgreSQL — repository pattern, typed queries, migrations
- Jest with ts-jest — running tests via `npm test` or `npx jest`
- Layered architecture: routes → controllers → services → repositories
- `Result<T>` pattern in services (`ok(value)` / `err({message, code})`) — no throwing in business logic
- `AppError` class for HTTP errors, centralized error middleware
...
- **Services return `Result<T>`** — never throw in business logic; use `ok(value)` / `err({message, code})`
- **Controllers are thin** — validate with Zod, call service, format response, throw `AppError` on error
- **Repositories wrap Prisma** — typed queries with `include` for related data
...
- **Tests:** Unit tests co-located in `src/**/__tests__/`; integration tests in `src/__tests__/`; fixtures in `src/__tests__/fixtures/`
