...
- `Result<T>` / `ok` / `err` service return pattern — service methods must never throw, return Result instead
- Zod schemas paired with TypeScript interfaces for API input validation (controller layer)
...
### TaskFlow API Pattern Checklist

When reviewing designs for this codebase, apply these additional checks:

**TypeScript Conventions:**
- [ ] Interfaces use PascalCase with no `I` prefix (e.g., `TaskService`, not `ITaskService`)
- [ ] No `any` types — flag as MAJOR; suggest `unknown` where type is genuinely uncertain
- [ ] Service method return types use `Promise<Result<T>>` pattern, not `Promise<T>` with throws
- [ ] Input DTOs co-located with Zod schemas (e.g., `taskCreateSchema` + `TaskCreateInput`)

**Layered Architecture:**
- [ ] Controllers only handle HTTP concerns — no business logic in controller contracts
- [ ] Services return `Result<T>` and never throw — verify service interface signatures
- [ ] Repositories encapsulate all Prisma queries — no Prisma types leak into service contracts
- [ ] New services accept repository interfaces via constructor (enables `jest.Mocked<T>`)

**Data Model Alignment:**
- [ ] DTOs are distinct from Prisma-generated model types
