...
- Jest with ts-jest — `describe`/`it`/`expect`, `beforeEach`/`afterEach`, `jest.fn()` and `jest.Mocked<T>`
- supertest for HTTP integration testing of Express endpoints
- Factory functions for test data (e.g., `taskFactory.build()`, `createMockTaskRepo()`)
- Prisma mock strategies — mock at the repository layer, not the Prisma client
- Testing the `Result<T>` pattern: assert `result.ok`, `result.value`, `result.error`
- Testing `AppError` throws from controllers, Zod validation errors
...
- **File location:** Co-locate unit tests at `src/<layer>/__tests__/<module>.test.ts`; integration tests in `src/__tests__/`
- **Mocking:** Use `jest.Mocked<T>` for typed mocks; create factory helpers like `createMockTaskRepo()` returning `jest.Mocked<TaskRepository>`
- **Test data:** Use factory functions (e.g., `taskFactory.build()`) from `src/__tests__/fixtures/` — do not hardcode raw objects in tests
- **Result<T> assertions:** Check `result.ok` first, then assert `result.value` or `result.error.code`/`result.error.message`
- **Async:** All service/repository tests are async; use `await` and `mockResolvedValue` / `mockRejectedValue`
- **Isolation:** Each `describe` block gets a fresh instance via `beforeEach`; never share mutable state across tests
- **Naming:** Test descriptions use domain language — `'should create task when user is project member'`, not `'T1.2 AC3'`
...
- **Fixtures:** `src/__tests__/fixtures/` — test data factories shared across test files
...
- **Validation:** Zod schemas in `src/models/`
