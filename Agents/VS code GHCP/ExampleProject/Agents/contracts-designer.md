...
- Zod for runtime validation schemas paired with TypeScript interfaces
...
When designing for this codebase, use these established patterns:

**TypeScript interfaces** (PascalCase, no `I` prefix) for all DTOs and public contracts:
```typescript
...
**Result type** for service methods (never throw in business logic):
```typescript
async createTask(input: TaskCreateInput, userId: string): Promise<Result<Task>>
```

**Zod schemas** paired with TypeScript interfaces for API input validation:
```typescript
const taskCreateSchema = z.object({ ... });
type TaskCreateInput = z.infer<typeof taskCreateSchema>;
```

**Repository interfaces** follow this pattern:
```typescript
interface TaskRepository {
  create(data: TaskCreateData): Promise<Task>;
  findByProject(projectId: string, pagination: PaginationInput): Promise<PaginatedResult<Task>>;
