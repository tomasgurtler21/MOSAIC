...
- Jest with ts-jest and supertest for HTTP integration testing
...
- Use of `any` type in new code — defeats strict TypeScript guarantees
- Hardcoded secrets, JWT keys, or database credentials in source files

The following patterns are **MAJOR**:
- Controller doing business logic instead of delegating to service — violates layered architecture
- Repository executing raw SQL bypassing Prisma schema — breaks ORM contract
- Missing `AppError` usage (catching and swallowing errors, or throwing plain `Error`) — inconsistent error handling
- Incorrect HTTP status codes returned from controllers

The following patterns are **MINOR**:
...
**TypeScript Patterns:**
- Interfaces preferred over type aliases for object shapes (e.g., `TaskCreateInput`, not `type TaskCreateInput = {...}`)
- `unknown` used instead of `any` when type is uncertain
- `strict: true` compliance — no implicit `any`, no unchecked nulls
- Enums follow PascalCase with UPPER_SNAKE_CASE members (e.g., `TaskStatus.IN_PROGRESS`)

**Express / Controller Patterns:**
- Controllers are thin: parse input with Zod schema → call service → format response
- All async route handlers must properly propagate errors (no unhandled promise rejections)
- Input parsed via `taskCreateSchema.parse(req.body)` — not manual property access

**Service Patterns:**
- Return `Result<T>` (using `ok(value)` / `err({message, code})`) — never throw in business logic
- Access database only via repository layer, never `prisma` directly
- Validate authorization (e.g., project membership) before mutating data

**Repository Patterns:**
- All database access via `this.prisma.<model>.<method>()` — no raw SQL unless explicitly required
- Pagination uses the shared `paginate()` utility from `src/utils/`
- `include` clauses kept minimal — only include what callers need
...
- `src/services/` — business logic, return `Result<T>`
...
- `src/utils/` — pagination, hashing, date formatting helpers
- `src/__tests__/` — integration tests with fixtures
- `prisma/schema.prisma` — database schema (entities: User, Project, ProjectMember, Task, Comment, Notification)
