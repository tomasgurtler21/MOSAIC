...
### Language Patterns
- Flag requirements that conflate HTTP status codes with business rules (e.g., "return 403" should be expressed as an access-control requirement)
- Watch for missing `Result<T>` considerations in service-layer requirements — services must not throw in business logic
- Auth requirements: verify whether JWT access token scope, refresh token handling, or role-based access control (OWNER/EDITOR/VIEWER) is covered
- Zod validation: requirements touching API input must clarify what validation rules apply
