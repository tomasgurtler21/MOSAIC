...
5. If `human_in_the_loop: true`, contact user to review judgments — the user may override judgments, provide additional context, or confirm the report
...
- TaskFlow API domain model: Users, Projects, Tasks, Comments, Notifications

This expertise helps you judge whether an attempted answer captures the correct semantics for the TaskFlow API, even when worded differently from the expected answer.
...
#### TypeScript Semantic Equivalence
When judging answers about the TaskFlow API, apply semantic equivalence for TypeScript-specific patterns:
- "returns a Result type" and "uses ok()/err() pattern" describe the same behavior — Match if key points are covered
- "throws AppError" vs "uses centralized error middleware" — these describe different behaviors; distinguish carefully
- Prisma-specific terminology vs. generic ORM terminology: accept both if the conceptual meaning matches
[[/INJECTION:LanguagePatterns]]

### Codebase Context

[[INJECTION:CodebaseContext]]
#### TaskFlow API Knowledge Areas
When assessing gap patterns in the verification report, use these domain areas for categorization:
- **Authentication & Authorization**: JWT lifecycle, refresh tokens, middleware, project membership
- **Task Management**: CRUD flows, status transitions, priority, assignment
- **Error Handling**: AppError, Result<T>, centralized middleware, service-layer conventions
- **Data Access**: Repository pattern, Prisma usage, transaction handling
- **Notification System**: event triggers, background jobs, delivery
- **API Layer**: Zod validation, controller thinness, route organization
[[/INJECTION:CodebaseContext]]
