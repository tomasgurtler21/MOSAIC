...
- **Parallel Tool Calls:** Issue multiple independent tool calls in a single response whenever possible (e.g., reading multiple files, running multiple diffs). Sequential tool calls are only permitted when a later call depends on the result of an earlier one.
