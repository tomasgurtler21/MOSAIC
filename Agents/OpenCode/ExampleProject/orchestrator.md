...
### Project Context — TaskFlow API
This orchestrator operates on the **TaskFlow API** codebase:
- **Stack:** Node.js 20 + Express 4 + TypeScript 5, PostgreSQL 16 via Prisma ORM, Jest test framework
- **Architecture:** Layered (routes → controllers → services → repositories)
- **Test commands:** `npm test` (all tests), `npm run test:coverage` (with coverage), `npm run test:integration` (integration)
- **Build commands:** `npm run build` (TypeScript), `npm run dev` (dev server), `npm run lint`, `npm run typecheck`
- **Key patterns:** Controller → Service → Repository; `Result<T>` return pattern in services; Zod for input validation; `AppError` for error handling
- Pass these context hints to subagents via `input_files` or `task_description` when relevant to their task scope.
