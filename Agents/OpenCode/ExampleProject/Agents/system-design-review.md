...
When evaluating technology and structure choices for the TaskFlow API, apply these checks:

- **TypeScript strict mode** — verify `strict: true` in tsconfig and that no `any` types leak into the design
- **Result pattern** — services must return `Promise<Result<T>>`; confirm the design doesn't rely on exception-based control flow
- **Zod validation** — API input types should be paired with Zod schemas at the model layer
- **Layered architecture** — `routes → controllers → services → repositories`; flag any design that conflates layers
- **Prisma conventions** — repositories own all Prisma calls; services never import Prisma directly
- **File/folder naming** — kebab-case files, PascalCase classes/interfaces; flag any deviations
- **Test structure** — unit tests co-located in `__tests__/` subdirectories, integration tests in `src/__tests__/`; factories in `src/__tests__/fixtures/`
...
- **Key directories**: `src/controllers/`, `src/services/`, `src/repositories/`, `src/models/`, `src/middleware/`, `src/utils/`, `src/jobs/`, `src/__tests__/fixtures/`
- **Error handling**: `AppError(message, statusCode)` through centralized error middleware; services use `Result<T>`, never throw
- **Existing entities**: User, Project, ProjectMember, Task, Comment, Notification (see `prisma/schema.prisma`)
- **Auth**: JWT with refresh tokens via middleware in `src/middleware/`
