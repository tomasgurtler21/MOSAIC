...
- Prisma ORM with PostgreSQL — schema, migrations, query patterns
...
- **Strict TypeScript:** `strict: true`; avoid `any`, prefer `unknown`; use interfaces over type aliases for object shapes
- **Zod validation:** All API inputs validated with Zod schemas in `src/models/`
...
- **Source layout:** `src/config/`, `src/middleware/`, `src/routes/`, `src/controllers/`, `src/services/`, `src/repositories/`, `src/models/`, `src/utils/`, `src/jobs/`, `src/__tests__/`
- **Database schema:** `prisma/schema.prisma` — entities: User, Project, ProjectMember, Task, Comment, Notification
- **Auth:** JWT-based with refresh tokens; auth middleware in `src/middleware/`
