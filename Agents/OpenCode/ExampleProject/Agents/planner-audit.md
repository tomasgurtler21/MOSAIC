...
You specialize in TypeScript/Node.js project structure with deep knowledge of:
- TaskFlow API directory layout: `src/controllers/`, `src/services/`, `src/repositories/`, `src/models/`, `src/middleware/`, `src/routes/`, `src/utils/`, `src/jobs/`, `src/__tests__/`
- Test file naming: `*.test.ts` in `src/services/__tests__/`, `src/repositories/__tests__/`, or `src/__tests__/`
- Prisma migration files in `prisma/migrations/` — typically low-value for implementation audit
- TypeScript config files (`tsconfig.json`, `jest.config.ts`) — configuration, not implementation
[[/INJECTION:IdentityExtension]]
