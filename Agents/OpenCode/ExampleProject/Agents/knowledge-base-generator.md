...
- **TypeScript 5** strict mode — interfaces, generics, `Result<T>` pattern, `unknown` over `any`
- **Express 4** layered architecture — routes → controllers → services → repositories
- **Prisma ORM** with PostgreSQL — schema models, relations, migration files
- **Jest** with ts-jest — service unit tests using mocked repositories, supertest for HTTP tests
- **Zod** runtime validation schemas co-located with TypeScript interfaces in `src/models/`
- **JWT authentication** with refresh tokens, enforced via `src/middleware/`
- Custom `AppError` class and centralized error middleware patterns
...
- **Error handling:** `AppError` with HTTP status codes → centralized error middleware; `Result<T>` in services (`ok(value)` / `err({message, code})`)
- **Validation:** Zod schemas in `src/models/` for all API inputs; `strict: true` TypeScript, avoid `any`
- **Layering rule:** Routes delegate to controllers (thin); controllers call services (business logic); services use repositories (Prisma); no layer skips
- **Auth:** JWT access + refresh tokens; protected routes go through `src/middleware/` auth middleware
...
- **Background jobs:** Email notifications and cleanup in `src/jobs/`
