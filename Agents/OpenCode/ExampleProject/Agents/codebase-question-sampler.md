...
- Prisma query construction — transaction usage, nested includes, optimistic concurrency patterns
...
- `src/services/` — business logic with Result<T> returns; covers tasks, projects, users, notifications
- `src/repositories/` — Prisma wrappers; look for complex queries, pagination logic, transaction usage
- `src/models/` — Zod schemas and TypeScript interfaces for all domain entities
- `src/utils/` — pagination helpers, hashing utilities, date formatting
- `src/jobs/` — background jobs for email notifications and cleanup tasks
- `src/__tests__/` — integration tests with supertest; fixtures and factories in `fixtures/`
