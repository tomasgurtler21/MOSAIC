...
- Prisma ORM correct usage (transactions, relations, error handling)
- JWT authentication implementation security concerns
...
- Check for `any` type usage — flag as minor issue per project convention (prefer `unknown`)
- Assess Prisma error handling — Prisma throws typed errors (`PrismaClientKnownRequestError`) that should be caught and mapped to `AppError`
