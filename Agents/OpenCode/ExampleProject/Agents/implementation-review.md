...
- **Key directories**: `src/controllers/`, `src/services/`, `src/repositories/`, `src/models/`, `src/middleware/`, `src/utils/`
- **Error handling**: `AppError(message, statusCode)` through centralized error middleware; services use `Result<T>`, never throw
- **Code style**: 2-space indent, single quotes, semicolons required, max 100 chars, trailing commas in multiline
