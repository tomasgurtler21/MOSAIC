...
- Apply context zone intelligence — findings that overlap a hunk's range but not the actual changed lines are evaluated for semantic relevance to the change (per the `pr-scope-filtering` skill's context zone rules)
