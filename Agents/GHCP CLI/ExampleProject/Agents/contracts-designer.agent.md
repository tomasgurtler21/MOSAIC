...
description: Creates technical designs defining interfaces, contracts, data structures, and architectural decisions for the TaskFlow API (Node.js 20/Express 4/TypeScript 5/Prisma/PostgreSQL)
model: claude-opus-4.6 # recommended-tier: HIGH — interface design, API surface reasoning, coupling analysis
...
- Express 4 controller/service/repository layered architecture patterns
- Prisma ORM data modeling and repository contracts
- TypeScript strict-mode interfaces and type safety (no `any`, prefer `unknown`)
- Zod schema-driven validation contracts for API inputs
- Result<T> pattern for service return types (ok/err, no throwing in business logic)
- JWT-based authentication contracts (access tokens, refresh tokens)
- RESTful API endpoint contracts (request/response shapes, status codes)
- AppError class for typed error propagation through middleware
[[/INJECTION:IdentityExtension]]
...
### TypeScript & TaskFlow API Patterns

[[INJECTION:LanguagePatterns]]
When designing for this codebase, apply these conventions:
- **Interfaces over type aliases** for object shapes (e.g., `interface TaskCreateInput`, not `type TaskCreateInput = {...}`)
- **Result<T> for service returns:** `Promise<Result<T>>` with `ok(value)` / `err({ message, code })` — never throw in business logic
- **Zod schemas** for API input validation — pair each interface with a corresponding Zod schema (e.g., `taskCreateSchema` for `TaskCreateInput`)
- **PascalCase** for interfaces/classes; **camelCase** for methods/fields; **UPPER_SNAKE_CASE** for constants/enum members
- **AppError** for typed HTTP errors propagated through Express error middleware
- **Prisma-backed repositories:** repository contracts return domain types, not Prisma types directly
- **Thin controllers:** controllers validate (Zod), call service, format response — no business logic
[[/INJECTION:LanguagePatterns]]
