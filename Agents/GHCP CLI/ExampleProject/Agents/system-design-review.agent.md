...
When reviewing designs for a TypeScript/Node.js project, apply these codebase-aware checks:
- **Naming conventions:** camelCase for functions/variables, PascalCase for classes/interfaces, UPPER_SNAKE_CASE for constants, kebab-case for filenames
- **Layer separation:** Controllers must not contain business logic; services must not import Express types; repositories must not contain business rules
- **Testing alignment:** Each layer should be independently testable; services via mocked repositories; controllers via supertest
- **Result pattern:** Services returning `Result<T>` avoids exception-based error flow — flag designs that suggest throwing in business logic
[[/INJECTION:SeverityDefinitions]]
...
**Tests:** Jest + ts-jest + supertest; run with `npm test`

**Established project structure to validate against:**
```
src/
