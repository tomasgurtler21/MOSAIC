...
- `jest.Mocked<T>` for typed mock repositories and services
- `beforeEach` setup with fresh mocks; `mockResolvedValue` / `mockReturnValue` for stubbing
...
- Integration tests go in `src/__tests__/`; fixtures in `src/__tests__/fixtures/`
- Use `jest.Mocked<RepositoryClass>` for typed mocks; create mock factories (e.g., `createMockTaskRepo()`)
- `beforeEach` to reset mocks; use `mockResolvedValue` for async stubs

**Testing the Result pattern:**
```typescript
const result = await service.createTask(input, userId);
expect(result.ok).toBe(true);
expect(result.value).toMatchObject({ title: input.title });
```
...
**Key entities for tests:** User, Project, ProjectMember, Task (status: TODO/IN_PROGRESS/REVIEW/DONE, priority: LOW/MEDIUM/HIGH/URGENT), Comment, Notification
