...
- `Result<T>` pattern using `ok`/`err` in service layer (no throwing in business logic)
- Custom `AppError` class with HTTP status codes and centralized error middleware
...
- Strict TypeScript (`strict: true`); avoid `any`, use `unknown`
- Prefer interfaces over type aliases for object shapes
- camelCase functions/variables, PascalCase classes/interfaces, UPPER_SNAKE_CASE constants
- kebab-case filenames (e.g., `task.service.ts`)

**Architecture patterns:**
- Controllers are thin: validate with Zod, call service, format response
- Services return `Result<T>` using `ok(value)` / `err({ message, code })` — never throw in business logic
- Repositories encapsulate all Prisma queries; no direct Prisma calls outside repositories
- Use `AppError` with HTTP status code for controller-layer errors; centralized error middleware handles propagation
...
**Stack:** Node.js 20 + Express 4 + TypeScript 5, PostgreSQL 16 via Prisma ORM, Jest + ts-jest
