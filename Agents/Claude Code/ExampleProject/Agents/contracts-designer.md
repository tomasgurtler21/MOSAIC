...
- Zod schema validation for API input contracts
...
- **Result pattern for services:** `Promise<Result<T>>` return types (never throw in business logic)
...
- **Services** hold business logic and return `Result<T>` — import from `src/services/`
- **Repositories** encapsulate all Prisma queries — import from `src/repositories/`
- **Models** contain Zod schemas + TypeScript interfaces — import from `src/models/`
