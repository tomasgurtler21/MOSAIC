...
- Jest testing framework with ts-jest and supertest for HTTP testing
...
- Use `AppError` class with HTTP status codes for error propagation through centralized error middleware
...
- **Supporting:** `src/utils/` (pagination, hashing, date formatting), `src/jobs/` (background jobs), `src/__tests__/` (integration tests + fixtures)
