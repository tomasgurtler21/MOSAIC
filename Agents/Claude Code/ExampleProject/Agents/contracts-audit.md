...
- TypeScript interface and type design (strict mode, no implicit `any`)
- Zod schema design for API input validation
- Service/repository contract patterns in Express layered architecture
- Prisma model and repository interface conventions
- Result type pattern: `Result<T>` with `ok()` / `err()`
...
- Interfaces use PascalCase (no `I` prefix): `TaskCreateInput`, `UserResponse`
- `Result<T>` return type on service methods — never `throws` in business logic
- Zod schemas in `src/models/` define runtime validation contracts
- Repository interfaces: `findById`, `findByProject`, `create`, `update`, `delete` naming conventions
- `PaginatedResult<T>` for list endpoints
- `AppError` with HTTP status codes as the error contract in controllers

### TaskFlow API Codebase
- **Stack:** Node.js 20 + Express 4 + TypeScript 5 (strict mode, no implicit `any`)
- **Database:** PostgreSQL 16 via Prisma ORM
- **Structure:** `src/models/` (Zod schemas + TypeScript interfaces), `src/services/`, `src/repositories/`, `src/controllers/`
- **Key entities:** User, Project, ProjectMember, Task, Comment, Notification
- **Conventions:** Prefer interfaces over type aliases for object shapes; use `unknown` when type is uncertain; trailing commas in multiline
