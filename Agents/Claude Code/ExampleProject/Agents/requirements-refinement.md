...
- Jest testing patterns (unit tests with mocked repositories, integration tests with supertest)
...
- Use `Result<T>` pattern in services (`ok(value)` / `err({ message, code })`) — no throwing in business logic
...
- **Auth:** JWT access tokens + refresh tokens; `req.user.id` available in authenticated routes
