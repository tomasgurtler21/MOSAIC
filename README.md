...
**The `Catalog/` folder in this repository is a reference example.** You're meant to create your own — with your own agents, workflows, and skills tailored to how you work. MOSAIC includes utility agents (a subagent creator, a workflow creator) that help you build these from scratch, using the same patterns the reference catalog demonstrates. The reference gets you running; your catalog is where the real value lives.

Everything in this repository — code, agents, design documents, workflows, this README — was generated by AI agents under human direction. Not as a novelty: this project is its own proof of concept. Specialized agents with defined responsibilities produced the work; a human architect made the design decisions, reviewed the output, and guided iteration across hundreds of hours. The quality you see here is what structured human-AI collaboration produces — and what MOSAIC is designed to enable for your projects.
...
**MOSAIC is markdown files.** Agents are markdown. Workflows are markdown tables. Artifacts are markdown. The tools (`mosaic-deploy`, `mosaic-run`, etc.) exist to manage those markdown files — assemble them, deploy them, drive runs — but the system itself is just text files that any harness can read and write.
...
│   └── RunnerGuide.md
│
├── Catalog/                         # Reference catalog — create your own with your agents, workflows, and skills
...
MOSAIC provides the framework and reference examples. You build the workflows and agents that are perfect for your work. We don't accept changes to the reference catalog — instead, we'd love your help with what makes the framework stronger: bug reports from real usage, harness knowledge, tooling improvements, and test coverage across harness/model combinations we can't run ourselves.

The single most valuable contribution is using MOSAIC and telling us what breaks.
