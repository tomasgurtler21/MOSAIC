...
**Call:** `read({filePath: "C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts\varied-content-test.txt", offset: 3, limit: 4})`
...
(Showing lines 3-6 of 20. Use offset=7 to continue.)
[/content]
--- RAW OUTPUT END ---
...
**Call:** `read({filePath: "C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts\does-not-exist.txt"})`

--- RAW OUTPUT START ---
File not found: C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts\does-not-exist.txt
--- RAW OUTPUT END ---

## read — large file with default parameters
...
**Call:** `glob({pattern: "*.xyz", path: "C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts"})`
...
## grep — no content match

**Call:** `grep({pattern: "NO_SUCH_PATTERN", path: "C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts"})`
...
**Call:** `grep({pattern: "EDIT TARGET", path: "C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts", include: "varied-content-test.txt"})`

--- RAW OUTPUT START ---
Found 2 matches
C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\TestArtifacts\varied-content-test.txt:
  Line 15: --- EDIT TARGET START ---

  Line 17: --- EDIT TARGET END ---
--- RAW OUTPUT END ---

## apply_patch — edit success

**Call:** `apply_patch({patchText: "*** Begin Patch\n*** Update File: .../temp-edit-test.txt\n...\n*** End Patch"})`

--- RAW OUTPUT START ---
Success. Updated the following files:
M HarnessKnowledge/SystemPromptCapture/OpenCode/PrimaryAgent/temp-edit-test.txt
--- RAW OUTPUT END ---

## apply_patch — expected text absent

**Call:** `apply_patch({patchText: "*** Begin Patch\n*** Update File: .../temp-edit-test.txt\n...THIS STRING DOES NOT EXIST...\n*** End Patch"})`

--- RAW OUTPUT START ---
apply_patch verification failed: Error: Failed to find expected lines in C:\AI\MOSAIC\MOSAIC\HarnessKnowledge\SystemPromptCapture\OpenCode\PrimaryAgent\temp-edit-test.txt:
THIS STRING DOES NOT EXIST
--- RAW OUTPUT END ---

## apply_patch — add-file success

**Call:** `apply_patch({patchText: "*** Begin Patch\n*** Add File: .../temp-write-test.txt\n+temporary write capture\n*** End Patch"})`

--- RAW OUTPUT START ---
Success. Updated the following files:
A HarnessKnowledge/SystemPromptCapture/OpenCode/PrimaryAgent/temp-write-test.txt
--- RAW OUTPUT END ---

## bash — successful command

**Call:** `bash({command: "$PSVersionTable.PSVersion.ToString()", workdir: "C:\AI\MOSAIC\MOSAIC"})`
...
**Call:** `bash({command: "exit 7", workdir: "C:\AI\MOSAIC\MOSAIC"})`
...
Full output saved to: C:\Users\tgurt\.local\share\opencode\tool-output\tool_077ef9664001kpkix2l687ry6L
...
## question — single-choice response

**Call:** `question({questions: [{header: "Capture test", question: "This is a tool-output-format test. Which option should be recorded?", options: [...]}]})`

--- RAW OUTPUT START ---
User has answered your questions: "This is a tool-output-format test. Which option should be recorded?"="Test option (Recommended)". You can now continue with the user's answers in mind.
--- RAW OUTPUT END ---

## task — minimal subagent completion

**Call:** `task({description: "Test task output", prompt: "...", subagent_type: "general", command: "system prompt capture test"})`

--- RAW OUTPUT START ---
[task id="ses_f8810195fffeNVs7rutBT63J8R" state="completed"]
[task_result]
TASK_OUTPUT_CAPTURE_OK
[/task_result]
[/task]
--- RAW OUTPUT END ---
...
**Call:** `webfetch({url: "https://example.com", format: "text"})`
...
**Call:** `webfetch({url: "https://example.invalid", format: "text"})`

--- RAW OUTPUT START ---
Transport error (GET https://example.invalid)
--- RAW OUTPUT END ---

## websearch — single search result

**Call:** `websearch({query: "OpenCode AI coding agent official 2026", numResults: 1, type: "fast"})`
...
...
connect any model from any provider, including Claude, GPT, Gemini and
...
### What is OpenCode?
...
...
## skill — load registered skill

**Call:** `skill({name: "lean-tdd"})`

--- RAW OUTPUT START ---
[skill_content name="lean-tdd"]
---
name: lean-tdd
...
Base directory for this skill: C:\AI\MOSAIC\MOSAIC\.claude\skills\lean-tdd
Relative paths in this skill (e.g., scripts/, reference/) are relative to this base directory.
Note: file list is sampled.

[skill_files]
[file]C:\AI\MOSAIC\MOSAIC\.claude\skills\lean-tdd\EXAMPLES-CSHARP.md[/file]
[/skill_files]
[/skill_content]
--- RAW OUTPUT END ---
...
[
  {
    "content": "Create the OpenCode PrimaryAgent raw system-prompt capture",
    "status": "completed",
    "priority": "high"
  },
  {
    "content": "Create the harness-only clean capture",
    "status": "completed",
    "priority": "high"
  },
  {
    "content": "Extract built-in tool definitions to JSON",
    "status": "completed",
    "priority": "high"
  },
  {
    "content": "Exercise built-in tools and capture raw outputs",
    "status": "in_progress",
    "priority": "high"
  },
  {
    "content": "Create structured tool-output schemas and verify all five files",
    "status": "pending",
    "priority": "high"
  }
]
...
**Call:** `parallel({tool_uses: [{recipient_name: "functions.glob", parameters: {...}}, ...]})`

--- RAW OUTPUT START ---
No standalone wrapper output. Each child tool returned its own ordinary result independently.
--- RAW OUTPUT END ---
